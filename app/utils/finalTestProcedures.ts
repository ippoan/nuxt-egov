...
export const PROCS_WITH_DESTINATION = new Set([
  ...
  // 構成管理情報に提出先情報が必須（e-Gov 実機エラー「提出先情報を設定しなければなりません」）
  '900A102800072000',
])

/**
 * 申請書様式とは別に「実添付書類」が必須の手続 → 添付書類名称のリスト。
 *
 * 個別署名形式で、申請書様式 (file_info) とは別に手続マスタが添付書類を必須とするもの。
 * e-Gov 実機エラー「添付必須のファイル(<添付書類名称>)が添付されていません」の括弧内が
 * 要求される添付書類名称。スケルトン構造 (file_info / check.xml) からは判別できず、
 * 実機エラーの逆引きで把握する（No.24 は不要・No.25 は必須など同系手続でも差がある）。
 *
 * 値は添付書類名称の配列。submitOne が main kousei.xml の添付書類属性情報に列挙し、
 * ダミー実ファイル (`<添付書類名称>.txt`) を ZIP に同梱する。
 */
